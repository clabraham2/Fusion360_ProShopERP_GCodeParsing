...
function proshopErp() {
  ...
  this.init = function () {
    properties["_useProshopGCodeParsing"] = {
      title: "ProShop ERP Sequence Detail",
      description:
        "Include information that can be parsed by ProShop ERP to fill in sequence detail.",
      group: "proShopERP",
      type: "boolean",
      value: true,
      scope: "post",
    ...
    properties["workOrderNumber"] = {
      title: "ProShop ERP Work Order Number",
      description: "The work order for the part routing in ProShop ERP.",
      group: "proShopERP",
      type: "string",
      value: "",
      scope: "post",
    };
    properties["operationNumber"] = {
      title: "ProShop ERP Operation Number",
      description: "The operation number for the part routing in ProShop ERP.",
      group: "proShopERP",
      type: "integer",
      value: 50,
      scope: "post",
    ...
    properties["partNumber"] = {
      title: "ProShop ERP Part Number",
      description: "The part number for the part routing in ProShop ERP.",
      group: "proShopERP",
      type: "string",
      value: "",
      scope: "post",
    };
    properties["rtaNumberSource"] = {
      title: "RTA Number Source",
      description: "Choose Tool Field used for Proshop RTA Number.",
      group: "proShopERP",
      type: "enum",
      values: [
        { title: "Tool Number", id: "toolNumber" },
        { title: "Tool Comment", id: "toolComment" },
      ],
      value: "toolNumber",
      scope: "post",
    };
    properties["detailFormat"] = {
      title: "Detail Format",
      description: "Choose Tool List or Sequence List.",
      group: "proShopERP",
      type: "enum",
      values: [
        { title: "Tool List", id: "toolList" },
        { title: "Sequence List", id: "sequenceList" },
      ],
      value: "toolList",
      scope: "post",
    };
  ...
  this.writeToolDetails = function () {
    if (
      getProperty("_useProshopGCodeParsing") &&
      getProperty("detailFormat") == "toolList"
    ) {
      var toolErpLengthFormat = createFormat({
      ...
      if (tools.getNumberOfTools() > 0) {
        ...
        for (var i = 0; i < tools.getNumberOfTools(); ++i) {
          ...
          writeComment(
            " * OUT OF HOLDER LGTH = " +
              toolErpLengthFormat.format(tool.bodyLength)
          );
        }
        writeln("");
      }
  ...
  this.writeSectionDetails = function () {
    if (
      getProperty("_useProshopGCodeParsing") &&
      getProperty("detailFormat") == "sequenceList"
    ) {
      var toolErpLengthFormat = createFormat({
        decimals: unit == MM ? 1 : 2,
        forceDecimal: true,
      ...
      writeComment(
        " * OUT OF HOLDER LGTH = " + toolErpLengthFormat.format(tool.bodyLength)
      );
    }
  };
